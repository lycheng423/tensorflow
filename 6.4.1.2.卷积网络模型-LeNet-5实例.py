...
# 在调整完输入格式之后，只需要在程序 mnist_inference.py 中实现类似 LeNet-5 模型结构的前向传播过程即可。
# 下面给出了修改后的 mnist_infemece.py 程序。
...
def inference(input_tensor, train, regularizer):
    ...
    # 使用全0填充且移动的步长为2。这一层的输入是上一层的输出，也就是28x28x32的矩阵。输出为 1444x32 的矩阵。
    with tf.name_scope('layer2-pool1'):
        pool1 = tf.nn.max_pool(
            relu1, ksize=[l, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')

    # 声明第三层卷积层的变量并实现前向传播过程，这一层的输入为 14x1032 的矩阵。 输出为 14x14x64 的矩阵。
    ...
    # 然而第五层全连接层需要的输入格式为向量，所以在这里需要将这个7><7X64的矩阵拉直成一个向量。
    # pool2.get_Shape函数可以得到第四层输出矩阵的维度而不需要手工计算。
    # 注意因为每一层神经网络输入输出都为一个batch的矩阵，所以这里得到的维度也包含了一个batch中数据的个数。
    ...
    # 声明第五层全连接层的变ffl并实现前向传播过程。
    # 这一层的输入是拉直之后的一组向量，向量长度为 3136, 输出是一组长度为 512 的向量。
    # 这一层和之前在第 5 章中介绍的基本一致，唯一的区别就是引入了dropout的概念。
    # dropout在训练时会随机将部分节点的输出改为 0。dropout 可以避免过拟合问题，从而使得模型在测试数据上的效果更好。
    ...
    # 只有全连接层的权重需要加入正则化。
    if regularizer != None:
        tf.add_to.collection('losses', regularizer(fc1_weights))
    ...
    # 声明第六层全连接层的变®并实现前向传播过程。这一层的输入为一组长度为 512 的向量，
