...
with tf.Session() as sess:
    ...
    # 通过ema.average(vl)获取滑动平均之后变量的取值。
    # 在初始化之后变量vl的值和vl的滑动平均都为0。
    print(sess.run([vl, ema.average(vl)]))
    # 输出[0.0,0.0]

    # 更新变量 vl 的值到 5。
    sess.run(tf.assign(vl, 5))
    # 更新vl的滑动平均值。衰减率为min{0.99, (l+step)/(10+step)= 0.1}=0.1,
    # 所以vl的滑动平均会被更新为0.1x0+0.9x5=4.5。
    ...
    # 更新vl的滑动平均值。衰减率为 min{0.99,(l+step)/(10+step) » 0.999}=0.99,
    # 所以vl的滑动平均会被更新为 0.99x4.5+0.01x10=4.555。
    sess.run(maintain_averages_op)
    print(sess.run([vl, ema.average(vl)]))
    # 输出 10.0, 4.5549998]
