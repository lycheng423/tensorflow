...
# tf.get_variable 函数与 tf.Variable 函数最大的区别在于指定变量名称的参数。
# 对于 tf.Variable 函数，变量名称是一个可选的参数，通过 name="v"的形式给出。
# 但是对于tf.get_variable 函数，变量名称是一个必填的参数。
# tf.get_variable 会根据这个名字去创建或者获取变量。
# 在上面的样例程序中，tf.get_variable 首先会试图去创建一个名字为 v 的参数,
# 如果创建失败(比如己经有同名的参数)，那么这个程序就会报错。
# 如果需要通过 tf.get_variable获取一个己经创建的变量，
# 需要通过tf.variable_scope函数来生成一个上下文 管理器，并明确指定在这个上下文管理器中，
# tf.get_variable 将直接获取己经生成的变量。
